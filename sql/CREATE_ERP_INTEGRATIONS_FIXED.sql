-- ============================================================================
-- ERP INTEGRATIONS SCHEMA (Compatible with existing CLOE schema)
-- ============================================================================
-- Purpose: Manage ERP system integrations and data synchronization
-- Version: 1.1 - Fixed to work with existing schema
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS erp_integrations (
    ...
    auth_type TEXT, -- 'api_key', 'oauth2', 'basic', 'certificate', 'token_based', 'none'
...
CREATE INDEX IF NOT EXISTS idx_erp_integrations_erp_system ON erp_integrations(erp_system);
...
CREATE INDEX IF NOT EXISTS idx_sync_history_date ON integration_sync_history(triggered_at DESC);
...
CREATE TABLE IF NOT EXISTS erp_account_mappings (
    ...
    -- System Side (maps to chart_of_accounts)
...
CREATE INDEX IF NOT EXISTS idx_account_mappings_integration ON erp_account_mappings(integration_id);
CREATE INDEX IF NOT EXISTS idx_account_mappings_erp_account ON erp_account_mappings(erp_account_code);
CREATE INDEX IF NOT EXISTS idx_account_mappings_coa ON erp_account_mappings(chart_of_accounts_id);
...
CREATE INDEX IF NOT EXISTS idx_integration_logs_sync ON integration_logs(sync_history_id);
...
-- Add columns to trial_balance for NetSuite metadata (if not exists)
DO $$ BEGIN
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES companies(id);
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS class TEXT;
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS sub_class TEXT;
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS netsuite_account_id TEXT;
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS netsuite_subsidiary_id TEXT;
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS gl_code TEXT;
    ALTER TABLE trial_balance ADD COLUMN IF NOT EXISTS gl_name TEXT;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

-- Add columns to chart_of_accounts for NetSuite metadata (if not exists)
DO $$ BEGIN
    ...
    ALTER TABLE chart_of_accounts ADD COLUMN IF NOT EXISTS netsuite_id TEXT;
    ALTER TABLE chart_of_accounts ADD COLUMN IF NOT EXISTS netsuite_account_type TEXT;
    ALTER TABLE chart_of_accounts ADD COLUMN IF NOT EXISTS gl_code TEXT;
    ALTER TABLE chart_of_accounts ADD COLUMN IF NOT EXISTS gl_name TEXT;
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_trial_balance_gl_code ON trial_balance(gl_code);
CREATE INDEX IF NOT EXISTS idx_chart_of_accounts_company ON chart_of_accounts(company_id);
CREATE INDEX IF NOT EXISTS idx_chart_of_accounts_gl_code ON chart_of_accounts(gl_code);
...
DROP POLICY IF EXISTS erp_integrations_company_policy ON erp_integrations;
DROP POLICY IF EXISTS integration_sync_history_company_policy ON integration_sync_history;
DROP POLICY IF EXISTS erp_account_mappings_company_policy ON erp_account_mappings;
DROP POLICY IF EXISTS integration_logs_company_policy ON integration_logs;
...
DROP TRIGGER IF EXISTS erp_integrations_updated_at ON erp_integrations;
CREATE TRIGGER erp_integrations_updated_at
    BEFORE UPDATE ON erp_integrations
    FOR EACH ROW
    EXECUTE FUNCTION update_integration_timestamp();

DROP TRIGGER IF EXISTS erp_account_mappings_updated_at ON erp_account_mappings;
...
('erp_templates', 'netsuite', '{"name": "Oracle NetSuite", "connection_fields": ["account_id", "realm"], "auth_type": "token_based"}', 'NetSuite connection template'),
...
DO $$
BEGIN
    RAISE NOTICE 'ERP Integrations schema created successfully!';
    RAISE NOTICE 'Tables created: erp_integrations, integration_sync_history, erp_account_mappings, integration_logs';
    RAISE NOTICE 'Columns added to: trial_balance, chart_of_accounts';
END $$;
