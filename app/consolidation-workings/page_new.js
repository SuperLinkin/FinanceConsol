...
export default function ConsolidationWorkings() {
  ...
  const buildCOAHierarchy = (masterHierarchy, coa, statementType) => {
    ...
    const classes = [...new Set(masterHierarchy
      .filter(h => allowedClasses.includes(h.class_name))
      .map(h => h.class_name))];
    ...
    classes.forEach(className => {
      ...
      subclasses.forEach(subclassName => {
        ...
        // Get notes for this subclass
        const notes = [...new Set(masterHierarchy
          .filter(h => h.class_name === className && h.subclass_name === subclassName)
          .map(h => h.note_name))];
        ...
        notes.forEach(noteName => {
          ...
          // Get subnotes and accounts
          const subnotes = masterHierarchy.filter(
          ...
          subnotes.forEach(subnote => {
            const subnoteNode = {
              id: `subnote-${className}-${subclassName}-${noteName}-${subnote.subnote_name}`,
              level: 'subnote',
              name: subnote.subnote_name,
              className: className,
              subclassName: subclassName,
              noteName: noteName,
              accounts: coa.filter(
                account =>
                  ...
                  account.subnote_level === subnote.subnote_name
              )
            };
  ...
  const renderRow = (node, depth = 0) => {
    ...
    const rowStyles = {
      ...
      subclass: 'bg-slate-100 font-semibold text-sm',
      note: 'bg-white text-sm',
      subnote: 'bg-slate-50 text-xs'
    };

    return (
      <React.Fragment key={node.id}>
        <tr className={`border-b border-slate-200 hover:bg-slate-50 ${rowStyles[node.level]}`}>
          {/* COA Name */}
          <td className="py-2 px-4 sticky left-0 bg-inherit z-10" style={{ paddingLeft: `${paddingLeft + 16}px` }}>
            <div className="flex items-center gap-2">
              ...
              {node.accounts && node.accounts.length > 0 && (
                <span className="text-xs opacity-60">({node.accounts.length})</span>
              )}
            </div>
          </td>
          ...
          {/* Entity Columns */}
          {entities.map(entity => (
            <td key={entity.id} className="py-2 px-4 text-right font-mono text-sm">
              {node.accounts && node.accounts.length > 0 ? (
          ...
          {/* Eliminations Column */}
          <td className="py-2 px-4 text-right font-mono text-sm bg-red-50">
            {node.accounts && node.accounts.length > 0 ? (
              <button
          ...
          {/* Adjustments Column */}
          <td className="py-2 px-4 text-right font-mono text-sm bg-blue-50">
            {node.accounts && node.accounts.length > 0 ? (
              <button
          ...
          {/* Consolidated Column */}
          <td className="py-2 px-4 text-right font-mono text-sm font-bold bg-indigo-50 sticky right-0">
            {node.accounts && node.accounts.length > 0 ? (
